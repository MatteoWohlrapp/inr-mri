...
def extract_patches(tensor, patch_size):
    ...
    patches = patches.transpose(1, 2).contiguous().view(batch_size, -1, patch_size, patch_size)
...
def reconstruct_image(patches, image_height, image_width, patch_size):
    ...
    assert patch_h == patch_size and patch_w == patch_size, "Patch size does not match expected dimensions."
    ...
    recon_images = F.fold(patches, output_size=(image_height, image_width), kernel_size=(patch_size, patch_size), stride=patch_size)
